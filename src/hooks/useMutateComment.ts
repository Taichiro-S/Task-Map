...
export const useMutateComment = () => {
  ...
  const createCommentMutation: any = useMutation(
    async (
      newComment: Omit<CommentData, 'id' | 'created_at' | 'updated_at'>,
    ) => {
      const previousComments = queryClient.getQueryData<
        Omit<CommentData, 'id' | 'created_at'>[]
      >(['comments'])
      if (previousComments) {
  ...
  const updateCommentMutation = useMutation(
    async (updatedComment: Omit<CommentData, 'created_at'>) => {
    ...
    if (previousComments) {
      queryClient.setQueryData(
        ...
        previouscomments.filter(
          (comment) => comment.comment_nanoid !== comment_nanoid,
        ),
      )
      // localStorage.setItem(
      //   'comments',
  ...
  const saveCommentMutation = useMutation({
    mutationFn: async (user_id: string | undefined) => {
      ...
      const commentDatas: Omit<CommentData, 'id' | 'created_at'>[] =
      ...
      const commentsToDelete = allComments.filter(
        (dbComment) =>
          !comments.find((comment) => comment.id === dbComment.comment_nanoid),
      )
